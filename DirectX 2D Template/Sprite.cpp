...
MeridorGraphics::Sprite::Sprite(DirectX::SpriteBatch* sprite_batch, RECT source, short pos_x, short pos_y, short width, short height, float depth, float rotation)
{
...
{
	if (DirectX::CreateWICTextureFromFile(device, file_name, nullptr, rc_view.ReleaseAndGetAddressOf()) != S_OK)
	{
		MessageBoxW(0, (static_cast<std::wstring>(L"Failed to load sprite: ") + file_name).c_str(), L"Sprite error", 0);
		exit(0);
	}
}
...
void MeridorGraphics::Sprite::SetX(short pos_x)
{
	this->pos_x = pos_x;
}
void MeridorGraphics::Sprite::SetY(short pos_y)
{
	this->pos_y = pos_y;
}
void MeridorGraphics::Sprite::SetWidth(short width)
{
	this->width = width;
}
void MeridorGraphics::Sprite::SetHeight(short height)
{
	this->height = height;
}
